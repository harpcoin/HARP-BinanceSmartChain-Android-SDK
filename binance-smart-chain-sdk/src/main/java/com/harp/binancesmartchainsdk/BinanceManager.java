...
import com.harp.binancesmartchainsdk.util.BalanceUtils;
import com.harp.binancesmartchainsdk.util.HarpUtils;
import com.harp.binancesmartchainsdk.util.Const;
import com.harp.binancesmartchainsdk.util.Erc20TokenWrapper;
import com.harp.binancesmartchainsdk.util.HyperLedgerApi;
import com.harp.binancesmartchainsdk.util.SubmitTransactionModel;
import com.harp.binancesmartchainsdk.util.Wallet;
import com.harp.binancesmartchainsdk.util.Token;
...
//        init("https://data-seed-prebsc-1-s1.binance.org:8545");
    ...
    public Single<Wallet> createWallet(String password, Context context) {
        return Single.fromCallable(() -> {
            ...
            try {
                ...
                String walletAddress = HarpUtils.generateNewWalletFile(password, new File(context.getFilesDir(), ""), false);
    ...
    public Single<String> importFromKeystore(String keystore, String password, Context context) {
        return Single.fromCallable(() -> {
            ...
            body.put("network", isMainNet() ? "MAINNET" : "TESTNET");
            try {
                Credentials credentials = HarpUtils.loadCredentials(password, keystore);
                String walletAddress = HarpUtils.generateWalletFile(password, credentials.getEcKeyPair(), new File(context.getFilesDir(), ""), false);
